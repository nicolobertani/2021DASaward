...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(7, 
           ...
           uiOutput(outputId = 'ui.bounds'),
           uiOutput(outputId = 'ui.final')
    )
  )
)
...
server <- function(input, output, session) {
  ...
  rv <- reactiveValues(keep.questioning = T, plot.fit = F)
  observeEvent(input$done, { rv$plot.fit = T})
  ...
  output$ui.bounds <- renderUI({
    if (rv$plot.fit == T) return()
    renderPlot({
  ...
  output$ui.final <- renderUI({
    if (rv$plot.fit == F) return()
    renderPlot({
      separator <- 8.3/10
      par(fig = c(0, 1, separator, 1), new = F,
          mar = c(0,0,0,0), oma = c(0,0,0,0), xpd = T)
      ...
      legend(x = .5, y = 1,
             legend = c("Lines - bounds:", "Upper bound", "Lower bound", "SVM fit"),
             lty = c(NA, 1, 1, 2),
             lwd = c(NA, 2, 2, 3),
             col = c(NA, cols[2:1], 'grey40'),
             text.font = c(1, 3, 3, 3),
             cex = 1, box.lty = "blank", y.intersp = 1
      )
